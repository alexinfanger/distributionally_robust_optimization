...
% \begin{lemma}
% The set $A_u\subseteq S$ is closed. 
% \end{lemma}
% \begin{proof}
% We will show the compliment is open. Pick any $x$ in the compliment, and let 
% \begin{align*}
% \epsilon \define \frac{\left(u-\sup_{t\in[0,T]} \left(\sqrt{\nu m_2}x(t)-\nu m_1 t\right)\right)}{2}>0.
% \end{align*}
% Now consider some $y\in S$ which is within $\epsilon$ of $x$. We have,
% \begin{align*}
% \sup_{t\in [0,T]}\left(\sqrt{\nu m_2}y(t)-\nu m_1 t\right)\leq \sup_{t\in[0,T]}\left(\sqrt{\nu m_2}x(t)-\nu m_1 t\right)+\sqrt{\nu m_2}\epsilon<u,
% \end{align*}
% where we have used the fact that $d_{J_1}(x,y)$ is weaker than the $\sup$ norm. 
% \end{proof}
