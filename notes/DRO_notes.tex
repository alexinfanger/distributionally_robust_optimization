...
\title{Notes on Distributionally Robust Optimization}
...
\end{align*}
It turns out that it is computationally intractable to deal with the above so we instead work with the following Brownian motion approximation,
\begin{align*}
...
In the numerical example, Jose and Karthyek Murthy, use the Paretto distribution for the claim sizes, with $\alpha=2.2$. This corresponds to a mean of $\frac{\alpha}{\alpha-1}$. The question
